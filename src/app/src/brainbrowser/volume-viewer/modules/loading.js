...
BrainBrowser.VolumeViewer.modules.loading = function(viewer) {
  ...
  */
  viewer.loadVolumes = function(options) {
    ...
    options = options || {};
    var blend_options = options.blend && typeof options.blend === "object" ? options.blend : {};
    ...
    function loadVolume(i) {
      setVolume(i, volume_descriptions[i], function() {
        ...
        if(options.blend && num_descriptions > 1) {
			viewer.createBlend(blend_options, function() {
  ...
  * ```
  */
  viewer.loadVolume = function(volume_description, callback, num) {
    setVolume(viewer.volumes.length, volume_description, callback, num);
  };
  ...
  */
  ...
  /**
  * @doc function
  * @name viewer.loading:createBlend
  ...
  *
  * @description
  * Create a blend of the currently loaded volumes.
  * ```js
  * viewer.createBlend({
  *   template: {
  *     element_id: "overlay-ui-template",
  ...
  * ```
  */
  viewer.createBlend = function(description, callback, num) {
  ...
  */
  ...
  viewer.syncPosition = function(panel, volume, axis_name) {
    ...
    viewer.volumes.forEach(function(synced_volume) {
      if (synced_volume !== volume) {
        ...
        synced_panel.volume.setVoxelCoords(vc.i, vc.j, vc.k);
  ...
  function setVolume(vol_id, volume_description, callback, vol_count) {
    openVolume(volume_description, function(volume) {
	  
	  //if placing the blended volume, delete the previous two
	  if(vol_id === 2) {
	    viewer.clearVolumes();
		vol_id = 0;
	  }
      ...
      volume.display = createVolumeDisplay(viewer.dom_element, vol_id, volume_description, vol_count);
  ...
  function createVolumeDisplay(dom_element, vol_id, volume_description, vol_count) {
    ...
    views.forEach(function(axis_name) {
      ...
      canvas.width = default_panel_width - 10;
      canvas.height = default_panel_height - 10;
      canvas.classList.add("slice-display");
	  canvas.style.zIndex = "0";
      canvas.style.backgroundColor = "#000000";
	  
	  var div = document.createElement("div");
	  div.classList.add("tbi-wrapper-div");
	  div.style.border = "1px solid #FFFFFF";
	  div.style.width  = default_panel_width.toString()  + "px";
	  div.style.height = default_panel_height.toString() + "px";
	
	  div.appendChild(canvas);
	  
	  //Drawing direction indicators aka landMarks
	  var landMarkDiv = document.createElement("div");
	  landMarkDiv.style.width = default_panel_width - 10 + "px";
	  landMarkDiv.style.height = default_panel_height - 10 + "px";
	  landMarkDiv.style.pointerEvents = "none";
	  landMarkDiv.style.zIndex = "1";
	  landMarkDiv.style.backgroundColor = "transparent";
	  landMarkDiv.classList.add("div-landmark");
	  
	  var landMarkWrapper = document.createElement("div");
	  landMarkWrapper.style.position = "relative";
	  
	  var marks = landMarks[axis_name];
	  
	  for(var i = 0; i < marks.length; i++) {
		var mark = marks[i];
		var markText = document.createElement("div");
		markText.style.display = "inline-block";
		markText.style.position = "absolute";
		markText.style.color = "#31708f";
		
		if(i == 0) {
		  markText.style.marginLeft = (default_panel_width-10)/2 + "px";
		}
		else if(i == 1) {
		  markText.style.marginLeft = (default_panel_width-25)   + "px";
		  markText.style.marginTop  = (default_panel_width-10)/2 + "px";
		}
		else if(i == 2) {
		  markText.style.marginLeft = (default_panel_width-10)/2   + "px";
		  markText.style.marginTop  = (default_panel_width-30) + "px";
		}
		else {
		  markText.style.marginTop  = (default_panel_width-10)/2 + "px";
		  markText.style.marginLeft = "5px";
		}
		
		var t = document.createTextNode(mark);
		markText.appendChild(t);
		landMarkWrapper.appendChild(markText);
	  }
	  
	  landMarkDiv.appendChild(landMarkWrapper);
	  div.appendChild(landMarkDiv);
      ...
      views.forEach(function(axis_name) {
        ...
        function drag(pointer, shift_key) {
          ...
          if(shift_key) {
			...
			/*TBIModif start trigger sliceupdate event here 
			  so the lesion map will update it's position*/
        ...
        function wheelHandler(event) {
		  ...
		  else
		  {
			  ...
			  if(panel.axis == "zspace")
				volume.setVoxelCoords(vc.i + delta, vc.j, vc.k);
		  
			  else if(panel.axis == "yspace")
				volume.setVoxelCoords(vc.i, vc.j + delta, vc.k);
		  
			  else if(panel.axis == "xspace")
				volume.setVoxelCoords(vc.i, vc.j, vc.k + delta);
				  
			  if (viewer.synced) {
				  var synced_volume = volume;
