...
// 	beta_proposal_std(g) = fabs(log(4.0*(sqrt(beta_b(g)/Sqr(beta_c(g))))));
	...
	for(int e = 1; e <= nevs; e++)
	  {
	    sumovere(t) += design.getdm()(t,e)*gamma_latest(e);
	    // 	OUT(gamma_latest(e));
	    // 	OUT(design.getdm()(t,e));
	  }	
      }
...
// 	OUT(t);
	...
	if(echanged>0)
	  {
	    sumovere(t) += (gamlatest-gamma_old)*design.getdm()(t,echanged);
	  }
...
//     ss_samples[0](samp) = sumsquare_residuals(design.getdm(),copedata,gamma_latest);
