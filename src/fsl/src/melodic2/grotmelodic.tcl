...
set VERSION [ exec ${FSLDIR}/bin/melodic -V | awk {{ print $3 }} - ]
...
$w.bhelp bind $w.tr -msg "The time (in seconds) between scanning successive FMRI volumes. This is only used to label time-series plots correctly - a value of 0 will report the time-series in volumes instead of seconds."
...
$w.bhelp bind $w.temp -msg "\"Highpass temporal filtering\" uses a local fit of a straight line
(Gaussian-weighted within the line to give a smooth response) to
remove low frequency artefacts. For Melodic it is often to remove 
global trends which will - if not removed - normally dominate the 
decomposition."
...
components from the data using Bayesian estimaters for the model 
...
the tails of the Gaussian) is fitted to the intensity histogram 
using an EM algorithm. 
From this mixture mode, Melodic calculates voxel-wise probabilities 
of 'activation' and only voxels above a certain threshold level are 
shown overlayed on top of an example volume. The default level of 
0.5 will report any voxel where the probability of belonging to the 
'activation' mixtures exceeds the probability of the voxel 
belonging to the background noise Gaussian."
...
pack $w.varnorm $w.dim $w.report $w.thresh -in $fmri(melodicf) -anchor w -side top
...
set total_volumes [ exec ${FSLDIR}/bin/avwnvols $INPUT 2> /dev/null ]

if { $fmri(ndelete) > 0 } {
    set total_volumes [ expr $total_volumes - $fmri(ndelete) ]
    feat5:exec "${FSLDIR}/bin/avwroi $INPUT prefiltered_func_data $fmri(ndelete) $total_volumes"
    set INPUT prefiltered_func_data
}

set target_vol_number [ expr $total_volumes / 2 ]
feat5:exec "${FSLDIR}/bin/avwroi $INPUT example_func $target_vol_number 1"
...
if { $fmri(st) > 0 } {
    ...
    feat5:exec "${FSLDIR}/bin/slicetimer -i $INPUT --out=prefiltered_func_data_st -r $fmri(tr) $st_opts"
...
if { $fmri(mc) != 0 } {
    feat5:exec "${FSLDIR}/bin/mcflirt -in $INPUT -out prefiltered_func_data_mcf -refvol $target_vol_number" C
...
if { $fmri(bet_yn) } {
    feat5:exec "${FSLDIR}/bin/betfunc $INPUT prefiltered_func_data_bet"
...
feat5:exec "$thecommand"

set INPUT filtered_func_data

#}}}

feat5:exec "rm -rf prefiltered_func_data*"
