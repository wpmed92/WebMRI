...
BandPassOptions* BandPassOptions::gopt = NULL;

void BandPassOptions::parse_command_line(int argc, char** argv, ofstream& logfile)
{
  ...
  while (n<argc) {
	...
	gopt->lowcut = atoi(argv[n]);
      else if(inp == 3)
	gopt->highcut = atoi(argv[n]);
      else if(inp == 4)
...
void BandPassOptions::print_usage(int argc, char *argv[])
{
  cout << "Usage: " << argv[0] << " [options] <groupfile> <lowcut> <highcut> <thresh>\n\n"
       << " Set highcut or low cut to zero if not needed.\n"
