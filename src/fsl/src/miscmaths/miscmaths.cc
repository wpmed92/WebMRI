...
void ols(const Matrix& data,const Matrix& des,const Matrix& tc, Matrix& cope,Matrix& varcope){
  ...
  Matrix sigsq=sum(NEWMAT::SP(res,res))/tR;
	     ...
	     float reltol)
{
  ...
  for (int n=1; n<=maxit; n++) {
	...
	if (rk2rk2<=0) {
      ...
      betak = rk1rk1 / rk2rk2;
      pk = rk1 + pk * betak;  // note RHS pk is p(k-1) in algorithm
    }  
    ...
    x = x + pk * alphak;  // note LHS is x(k) and RHS is x(k-1) in algorithm
    rk2 = rk1;  // update prior to the next step
    rk1 = rk1 - apk * alphak;  // note LHS is r(k) in algorithm
  }
  return 0;
}
...
// 	OUT(gam_m(1));
	...
	SymmetricMatrix tmp = lambda_B + ZZ*gam_y;
	lambda_B << tmp;
