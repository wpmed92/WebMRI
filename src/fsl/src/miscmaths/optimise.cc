...
namespace MISCMATHS {
			...
			const ColumnVector &unitdir, const ColumnVector &pt)
  {
    ...
    if (y1==0)  y1 = (*func)(unitdir*x1 + pt);
    if (ymid==0)  ymid = (*func)(unitdir*xmid + pt);
		  ...
		  float init_value, float boundguess) 
  {
    ...
    if (init_value==0.0) ymid = (*func)(unitdir*xmid + pt);
    else ymid = init_value;
    y1 = (*func)(unitdir*x1 + pt);
