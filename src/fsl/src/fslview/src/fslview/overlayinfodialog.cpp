...
{   
  ...
  if(!metaImage) {
    ...
    LookUpTable::Handle         curLut = disp->inqLookUpTable();
    Image::Handle               curModImg = metaImage->getDs()->inqModImage();
    ...
    std::for_each(m_imageGroup->beginLutList(),
                  ...
                  AddLutToCombo(m_lutCombo,curLut));
    ...
    std::for_each(m_imageGroup->begin(),
                  m_imageGroup->end(),
                  AddModImageToCombo(m_modCombo,curModImg));

    MetaImage::Handle metaImage = m_overlayList->getActiveMetaImage();

    if(metaImage) 
      {            
        int mode = metaImage->getDs()->inqDtiDisplay();
        switch (mode)
          {
          case DtiDisplay(None):
            m_dtiCombo->setCurrentItem(0);
            m_lutCombo->setEnabled(true);
	    m_negLutCombo->setEnabled(disp->inqSecondaryLookUpTable());
	    m_negativeLuts->setChecked(disp->inqSecondaryLookUpTable());
...
void OverlayInfoDialog::slutBoxChecked(bool state)
{
  ...
  if(state) lut = m_imageGroup->getLut(m_negLutCombo->currentItem());
...
{
  QString fn = QFileDialog::getOpenFileName( QString::null, "LUTs (*.lut *.rgb)", this );
  if ( !fn.isEmpty() ) {
