...
void SingleSeriesWidget::axisDisplay()
{ 
  TRACKER("SingleSeriesWidget::axisDisplay");
  if(m_enabled)
    {
      m_axisDisplay = !m_axisDisplay;
...
void SingleSeriesWidget::demeanButtonToggle(bool state)
{  
  TRACKER("SingleSeriesWidget::demeanButtonToggle");  
  setDemean(state);
  redraw();
}

void SingleSeriesWidget::percentButtonToggle(bool state)
{  
  TRACKER("SingleSeriesWidget::demeanButtonToggle");  
  setPercent(state);
  redraw();
      ...
      if(!inqCausedCursorUpdate())
        {
		  ...
		  TimeSeries::Handle timeSeries(model->getDataTimeSeries(x, y, z));
...
bool PlotOptions::inqFeatMode() {return m_impl->m_modelFit;}
