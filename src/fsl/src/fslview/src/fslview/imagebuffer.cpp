...
struct ColorPicker
{
  ...
  {
    const ColorTriplet& col = m_lut->inqValueIndex(v);
    m_rd = *col; m_gr = *(col+1); m_bl = *(col+2);
    m_al = (v > 0.0) ? 255 : 0;
  }
  ...
  inline void byLookUp(float v)
  {
    ...
    const ColorTriplet& col = m_lut->inqValue(adjustedValue);
    m_rd = *col; m_gr = *(col+1); m_bl = *(col+2);
	...
	{ 
	  ...
	  for( unsigned int x = 0; x < width; ++x) 
	    {              
	      ...
	      const ColorTriplet& col = lut->inqValueIndex(origValue);
	      rd = *col; gr = *(col+1); bl = *(col+2);
	...
	{ 
	  ...
	  for( unsigned int x = 0; x < width; ++x) 
	    {              
	      ...
	      const ColorTriplet& col = lut->inqValue(adjustedValue);
	      rd = *col; gr = *(col+1); bl = *(col+2);
