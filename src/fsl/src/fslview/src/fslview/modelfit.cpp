...
struct ModelFit::Implementation {
  ...
  Implementation(const QString& fd):
    m_featDir(fd), m_showFull(false), m_showPartial(false), m_nevs(0), m_nftests(0),
    m_higherLevel(false)
  {
    m_curCope=1;
    m_curPE=0;
    ...
    if(FileManager::checkFileExists(m_featDir + "/design.con"))
      readConFile(m_featDir + "/design.con");
    if(FileManager::checkFileExists(m_featDir + "/design.fts"))
      readFtsFile(m_featDir + "/design.fts");
    loadFiltFuncData(m_featDir + "/filtered_func_data");
    loadPeData(m_featDir + "/stats/pe");

    m_higherLevel = FileManager::checkFileExists(m_featDir + "/design.lev");
  }
...
void ModelFit::showFull(bool y) {  m_impl->m_showFull=y; notify(); }
bool ModelFit::showFull(void) const { return m_impl->m_showFull; }

void ModelFit::showPartial(bool y) {  m_impl->m_showPartial=y; notify(); }
bool ModelFit::showPartial(void) const { return m_impl->m_showPartial; }
