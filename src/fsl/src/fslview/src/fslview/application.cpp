...
/****************************************************************************
** $Id: application.cpp,v 1.136 2006/04/02 13:59:40 flitney Exp $
**
...
ApplicationWindow::ApplicationWindow(const OverlayOptionList& overlays)
  : ApplicationWindowBase( 0, "FslView", WDestructiveClose ), m_properties(Properties::create())
{  
...
# include "icons/fsllogo.xpm"
  ...
  if(!overlays.empty())
    {
      OverlayOptionList::const_iterator it = overlays.begin();
	...
	if( ModelFit::isFeatDir(fn) )
	  loadFeat(fn);
	if(it->lutSpecified())
	  ds->setLookUpTable(m_imageGroup->getLut(it->lutname()));
	...
	setCurrentDir(overlays.begin()->fileInfo().dirPath());
      } else
	QMessageBox::warning( this, "FSLView",
 			      "Failed to load base image. Command line processing aborted." );
    }
  
  // Initializing Command for FileMenu items
  //m_fileOpen = new FileOpen(this);
}
...
//!
//! This method is used to load a given file into the application. It can be called
//! from event handlers as well as directly from client code.
//! 
...
{ 
  ...
  try
  {
    ...
    {
      emit message(QString("Loading.... %1").arg(fn), -1);  
      // display message before file is loaded. -1 displays message until the next message
      // displayed or until the clear() function is called

      m_imageGroup = ImageGroup::create( Image::load((const char *)fn) );
      ImageInfo::Handle info(m_imageGroup->getMainImage()->getInfo());
      m_cursor = Cursor::create(info->inqX(),
                                info->inqY(),info->inqZ(), 
                                info->inqNumVolumes());
      ...
      emit message("Image Loaded", 10000); // display message after file has been loaded
...
void ApplicationWindow::helpAbout()
{
  QMessageBox::about( this, "FSLView",
		      QString("<h2>FSLView</h2><hr><b>Version %1.%2</b><br>"
			      "Written by:"
			      "<ul><li>Dave Flitney</li>"
			      "<li>James Saunders</li>"
			      "<li>Mark Jenkinson</li>"
			      "<li>Steve Smith</li>"
			      "<li>V Rama Aravind</li></ul><br>"
			      "Copyright(c) 2004 University of Oxford<br><hr>"
			      "Help pages:<br>"
			      "<b>html://www.fmrib.ox.ac.uk/fsl/fslview</b><hr>"
...
{
  QString fn(Preferences::instance()->mni152().c_str());
...
void ApplicationWindow::applicationProperties()
{
  //  Properties& p = PropertiesDialogImpl::getProperties(this);
}
...
{
  ViewWidget *view = (ViewWidget*)m_ws->activeWindow();
  OverlayList::Handle ol = view->getOverlayList();
  QString fn;
    ...
    if ( !fn.isEmpty() && checkFilesExist(fn,false) ) 
      {
        ...
        emit message( QString("Loading.... %1").arg(fn), 0);
...
// 	    LookUpTable::Handle lut;
// 	    // Choose lut in image file
// 	    if(!overlay->getInfo()->inqLutName().empty())
// 	      lut = m_imageGroup->getLut(overlay->getInfo()->inqLutName());
// 	    // look for "stat/mask" in the filename and choose colour lut if true
// 	    if ( overlay->getInfo()->isStatImage() || overlay->getInfo()->isMaskImage() )
// 	      {
// 		if(!lut) { lut = m_imageGroup->getNextLut(); }
// 	      }
// 	    activeOverlayList()->setLookUpTable(lut);
          }
        else
          {
            QMessageBox::warning( this, "FSLView",
      ...
      *********************************/
      ...
      if(info->inqNumVolumes()>1)
      {
        ...
        if(cd->m_create4dMask->isChecked())
          m_imageGroup->addOverlay(image->cloneStructure());
        else
          m_imageGroup->addOverlay(image->clone3dStructure());
      ...
      //ol->getActiveMetaImage()->getDs()->setLookUpTable(m_imageGroup->getLut(0));
      emit message( QString("Created new mask: %1-mask")
                    .arg(image->getInfo()->inqImageName().c_str()), 2000 );
      /**
...
OverlayList::Handle ApplicationWindow::activeOverlayList()
{
  ...
  if (!windowListEmpty())ol = view->getOverlayList();
  return ol;
}
...
void ApplicationWindow::viewShow(ViewWidget* v)
{
  if(windowListEmpty())
    { 
      v->showMaximized();
...
bool ApplicationWindow::tarnishCheck()
{
 ...
 bool result(true);
  ...
  if(m_imageGroup->inqTarnished())
    {
         switch(QMessageBox::warning( this,
				      "FSLView", QString("Continuing this action will lose unsaved data."),
				      "Cancel","OK","",0,0))
          {
          case 0:result = false;break;
          case 1:result = true;break;
...
void ApplicationWindow::loadFeat(QString &fn)
{
  QFileInfo  fi(fn);
  ...
  try {
    ...
    Image::Handle image = m_imageGroup->getMainImage();
  
    TimeSeriesWidget* timeseries = new TimeSeriesWidget(m_ws, image, m_cursor, m_modelFit);
    connect( timeseries, SIGNAL(windowClose(QCloseEvent*)), this, SLOT(childWindowClose(QCloseEvent*)));
