  ...
  m_impl(new Implementation())
{
  ...
  m_impl->m_imgDataList.push_back(createImageData(ol->getMainMetaImage()));
...
bool ImageDataStore::currentEmpty()
{
  ...
  if(m_impl->m_listPos >= m_impl->m_imgDataList.size())
    ...
    (m_impl->m_imgDataList[m_impl->m_listPos]->getBuffer().get())
    {result = false;}
...
{
  return m_impl->m_imgDataList[m_impl->m_listPos];
}
