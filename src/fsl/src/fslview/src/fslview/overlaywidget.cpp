...
{
 public:
  LayerListItem(QListView* parent, const  MetaImage::Handle mi):QListViewItem(parent),m_mi(mi){refresh();};
  ...
  float inqTransparency()        { return m_mi->inqTransparency();}  
...
void LayerListItem::refresh()
{
  setText(2,inqImageName().c_str());
  setText(3,QString::number(inqTransparency()));
  if(inqVisibility()){setPixmap(0,QPixmap(eye));}
  else               {setPixmap(0,NULL);}
  if(inqReadOnly())  {setPixmap(1,QPixmap(padlock));}
  else               {setPixmap(1,NULL);}
  if(inqIsMainImage()){setPixmap(4,QPixmap(mainimage));}
  else                {setPixmap(4,NULL);}
}
...
//   m_overlayListView->addColumn("Visible",-1);
//   m_overlayListView->addColumn("Locked",-1); 
//   m_overlayListView->addColumn("Filename",-1); 
//   m_overlayListView->addColumn("Transparency",20);
//   m_overlayListView->addColumn("Master",20);
...
void OverlayWidget::update(const OverlayList* l, OverlayListMsg msg)
{
  ...
  switch(msg)
  {
   case OverlayListMsg(Select):       updateControls();break;
   ...
   case OverlayListMsg(Order):        updateListView();updateControls();
...
void OverlayWidget::listDoubleClicked(QListViewItem* item)
{
 ...
 bool state;
  ...
  if(state)m_overlayListView->clearSelection();
}
