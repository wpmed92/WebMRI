...
int  nifti_fileexists(const char* fname)
{
   ...
   if( !znz_isnull(fp) )  { znzclose(fp);  free(fp); return 1; }
   free(fp);
   return 0;
}
...
#ifdef HAVE_ZLIB
   if (strcmp(basename + blen-7,".nii.gz") == 0) 
       { basename[blen-7]='\0'; return basename; }
   if (strcmp(basename + blen-7,".hdr.gz") == 0) 
       { basename[blen-7]='\0'; return basename; }
   if (strcmp(basename + blen-7,".img.gz") == 0) 
       { basename[blen-7]='\0'; return basename; }
#endif
   if (strcmp(basename + blen-4,".nii") == 0) 
       { basename[blen-4]='\0'; return basename; }
   if (strcmp(basename + blen-4,".hdr") == 0) 
       { basename[blen-4]='\0'; return basename; }
   if (strcmp(basename + blen-4,".img") == 0) 
       { basename[blen-4]='\0'; return basename; }
   return basename;
}
...
nifti_image *nifti_image_read( const char *hname , int read_data )
{
   ...
   if( znz_isnull(fp) ) { znzclose(fp);     ERREX("can't open header file") ; }
...
nifti_image * nifti_copy_nim_info(const nifti_image* src)
{
  ...
  if (src->fname != NULL)  { dest->fname = (char *)calloc(1,strlen(src->fname)); }
  if (src->iname != NULL)  { dest->iname = (char *)calloc(1,strlen(src->iname)); }
  dest->data = NULL;
  return dest;
}
