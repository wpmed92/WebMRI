...
{
  // calculates separate left and right gradients (with copying when at borders
  //  of the mask) or zero for points outside of the mask
  ...
  for (int z=im.minz(); z<=im.maxz(); z++) {
	...
	if (mask(x,y,z)>0.5) {
	  ...
	  if (x>im.minx()) {
	    if (mask(x-1,y,z)>0.5) {
	      grad[0](x,y,z) = im(x-1,y,z) - im(x,y,z);
	    }
	  }
	  // right gradient
	  if (x<im.maxx()) {
