...
void help()
{
  cerr<< endl << " pngappend  -  append PNG files horizontally and/or vertically into a new PNG file" << endl;
  usage();
}
...
int main(int argc, char *argv[])
{
  ...
  if((im_read = gdImageCreateFromPng(in))==NULL){
    cerr << endl <<argv[argidx-1] << " is not a valid png file" << endl;
    fclose(in);
    exit(1);
  };
  fclose(in);
  ...
  im_out = gdImageCreateTrueColor(im_read->sx,im_read->sy);
  ...
  while(argidx < argc-1){
    ...
    if((im_read = gdImageCreateFromPng(in))==NULL){
      cerr << endl <<argv[argidx] << " is not a valid png file" << endl;
      fclose(in);
      exit(1);
    };
    fclose(in);
  ...
  //output im_out
  FILE *pngout;
  if((pngout = fopen(argv[argc-1], "wb"))==NULL){
    cerr << endl <<"ERROR: cannot open " << argv[argc-1] << "for writing" << endl;
    exit(1);
  }
  gdImagePng(im_out, pngout);
  gdImageDestroy(im_out);
  fclose(pngout);
