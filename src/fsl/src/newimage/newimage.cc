...
//        lims[5]=Min(this->zsize() - 1,lims[5]); 
  ...
  minmaxstuff<T> calc_minmax(const volume<T>& vol, const volume<T>& mask)
  {
    ...
    for (int z=vol.minz(); z<=vol.maxz(); z++) {
	  ...
	  if (mask.value(x,y,z)>(T) 0.5) {
            ...
            else if (!valid || (val>newmax)) { newmax=val;  newmaxx=x; newmaxy=y; newmaxz=z;}
	    valid=true;
	  }
        }
      }
		     ...
		     T& min, T& max)
  {
    ...
    int i, validsize=0;
    T dtmin=vol.min(), dtmax=vol.max();

    /* zero histogram */
    for(i=0; i<bins; i++)
      hist(i+1)=0;
    
    if (min==max) /* set min and max if not already set */
      {
	min=dtmax;  max=dtmin;
      }
	...
	for (int x=vol.minx(); x<=vol.maxx(); x++) {
	  T volval=vol(x,y,z);
	  hist(Max(0, Min( (int)(fA*(volval) + fB), bins-1) ) + 1)++;
	  validsize++;
	}
      }
    }
		     ...
		     T& min, T& max, const volume<T>& mask)
  {
    ...
    int i, validsize=0;
    T dtmin=vol.min(mask), dtmax=vol.max(mask);
    ...
    for (int z=vol.minz(); z<=vol.maxz(); z++) {
      for (int y=vol.miny(); y<=vol.maxy(); y++) {
	for (int x=vol.minx(); x<=vol.maxx(); x++) {
	  T maskval=mask(x,y,z), volval=vol(x,y,z);
	  if ( (maskval>(T) 0.5) )
	    {
	      hist(Max(0, Min( (int)(fA*(volval) + fB), bins-1) ) + 1)++;
	      validsize++;
		     ...
		     T& min, T& max, const volume4D<T>& mask)
  {
	...
	for (int y=vol.miny(); y<=vol.maxy(); y++) {
	  for (int x=vol.minx(); x<=vol.maxx(); x++) {
	    T maskval=mask(x,y,z,Min(t,mask.maxt())), volval=vol(x,y,z,t);
	    if ( (maskval>(T) 0.5) )
	      {
		hist(Max(0, Min( (int)(fA*(volval) + fB), bins-1) ) + 1)++;
		validsize++;
		     ...
		     T& min, T& max, const volume<T>& mask)
  {
	...
	for (int y=vol.miny(); y<=vol.maxy(); y++) {
	  for (int x=vol.minx(); x<=vol.maxx(); x++) {
	    T maskval=mask(x,y,z), volval=vol(x,y,z,t);
	    if ( (maskval>(T) 0.5) ) 
	      {
		hist(Max(0, Min( (int)(fA*(volval) + fB), bins-1) ) + 1)++;
		validsize++;
  ...
  while ( (pass==1) ||
      ...
      if (pass>1) // redo histogram with new min and max
	{
	  ...
	  tmpmin = (T)( min + ((double)bottom_bin/(double)(HISTOGRAM_BINS))*(max-min) );
      ...
      if (pass==MAX_PASSES)  // give up and revert to full range ...
	{
      ...
      if (use_mask) {
	validsize = find_histogram(vol,hist,HISTOGRAM_BINS,min,max,mask);
      } else {
	validsize = find_histogram(vol,hist,HISTOGRAM_BINS,min,max);
      }
      
      if (validsize<1)
	{
	  min=max=thresh2=thresh98=0;
  ...
  int volume<T>::left_right_order() const
  {
    ...
    float det=-1.0;
    if (this->sform_code()!=NIFTI_XFORM_UNKNOWN) { 
      det = this->sform_mat().Determinant(); 
    } else if (this->qform_code()!=NIFTI_XFORM_UNKNOWN) { 
      det = this->qform_mat().Determinant(); 
    }
...
//        lims[6]=Min(this->zsize() - 1,lims[6]); 
  ...
  int volume4D<T>::copyproperties(const volume4D<T>& source) {
	...
	vols[t].copyproperties(source[Min(source.tsize()-1,t)]);
      ...
      // Only copy ROIs
      if (!samesize(source,*this)) {
	cerr << "Warning: non-equal ROIs in copyproperties" << endl;
      }
      int toffset = source.mint() - this->mint();
      for (int t=this->mint(); t<=this->maxt(); t++) {
	vols[t].copyproperties(source[t + toffset]);
      }
    }
    return 0;
  }
