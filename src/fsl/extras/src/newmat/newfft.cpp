...
//    GENERAL RADIX ONE DIMENSIONAL FOURIER TRANSFORM;
   ...
   for (int i = 0; i < N_FACTOR; i++)
   {
      ...
      {
      case 1: break;
      case 2: R_2_FTK (N,M,X,Y,X+M,Y+M); break;
      case 3: R_3_FTK (N,M,X,Y,X+M,Y+M,X+2*M,Y+2*M); break;
      case 4: R_4_FTK (N,M,X,Y,X+M,Y+M,X+2*M,Y+2*M,X+3*M,Y+3*M); break;
      case 5:
      ...
      case 16:
         R_16_FTK (N,M,X,Y,X+M,Y+M,X+2*M,Y+2*M,
            ...
            X+15*M,Y+15*M);
         break;
      default: R_P_FTK (N,M,P,X,Y); break;
      }
   }
...
//    RADIX THREE FOURIER TRANSFORM KERNEL
{
   ...
   L200:
      ...
      for (K=K0; K<N; K+=M3)
      {
         ...
         else { X1[K]=RA+IB; Y1[K]=IA-RB; X2[K]=RA-IB; Y2[K]=IA+RB; }
      }
      goto L100;
   L600: ;
   }
...
bool FFT_Controller::CanFactor(int PTS)
{
   ...
   while (N > 1)
   {
      ...
      for (int J = F; J <= PMAX; J++)
         if (N % J == 0) { fail = false; F=J; break; }
      if (fail || P >= NP || Q >= NQ) return false;
      N /= F;
...
{
   for (int k = 0; k < n; k++) { Value[k] = 0; product *= Radix[k]; }
}
...
void MultiRadixCounter::operator++()
{
   ...
   for (int k = 0; k < n; k++)
   {
      ...
      if (Value[k] == Radix[k]) { Value[k] = 0; reverse -= p; p = p1; }
      else return;
   }
   finish = true;
}
