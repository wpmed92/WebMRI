    ...
    var color_map = {
      ...
      */
      mapColors: function(intensity_values, options) {
		...
		/*TBIModif start*/
		var rawvalues = options.getraw === undefined ? false : options.getraw;
		/*TBIModif end*/
        ...
        for (i = 0, count = intensity_values.length; i < count; i++) {
          ...
          if(color_map_index < 0) {
			...
			/*TBIModif rawvalues*/
            destination[ic]     = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * default_colors[idc]     + brightness);
            destination[ic + 1] = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * default_colors[idc + 1] + brightness);
            destination[ic + 2] = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * default_colors[idc + 2] + brightness);
            destination[ic + 3] = scale    * default_colors[idc + 3];
          } else {
            // contrast includes scaling factor
            destination[ic]     = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * color_map_colors[color_map_index]     + brightness);
            destination[ic + 1] = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * color_map_colors[color_map_index + 1] + brightness);
            destination[ic + 2] = (rawvalues) ? ((value>255) ? 255 : value) : (contrast * color_map_colors[color_map_index + 2] + brightness);
